...
public class Main {
	...
	public static void main(String[] args) {
		...
		int x = 2;
		int y = 0;
		System.out.println("Num of elements in component which (" + x + ", " + y + ") is part of is: " + mutuallyAgreeable(x, y));
	}

	public static int mutuallyAgreeable(int x, int y){
		int mat[][]={
			{4, 2, 2},
			{6, 3, 6},
			{6, 4, 2}
		...
		for(int i = 0; i<n;i++){
			for(int j = 0; j<m; j++){
				for(int k = 0; k<n; k++){
					for(int l = 0; l<m; l++){
						if(NZD(mat[i][j], mat[k][l]) == 1){
							graf.addEdgeInNonOriented(
		...
		graf.printGraph();
		System.out.println();
		int num = graf.allElOfComponent(pairInNode.get(new Pair(x,y)));
		return num;
	}
