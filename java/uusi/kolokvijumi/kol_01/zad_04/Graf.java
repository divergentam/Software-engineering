...
public class Graf {
	...
	public Graf(int n){
		...
		for(int i = 0;i<= numNodes; i++){ // za nultu listu nemamo susede jer to nije cvor
			nodes.add(new LinkedList<Integer>());
		}
	}

	public void addEdgeInNonOriented(int start, int end, int weight){
		if(!doesExist(start,end, weight)){
			numEdges++;
			edges.add(new Edge(start,end,weight));
			nodes.get(start).add(edges.size() - 1);
		}
		if(!doesExist(end,start, weight)){
			numEdges++;
			edges.add(new Edge(end,start,weight));
			nodes.get(end).add(edges.size() - 1);
		}
	}

	private boolean doesExist(int start, int end, int weight) {
		boolean exist = false;
		for(int i = 0; i<edges.size(); i++){
			if(start == edges.get(i).pocetak && end == edges.get(i).kraj && weight == edges.get(i).weight){
				exist = true;
			}
		}
		return exist;
	...
	public int allElOfComponent(int node){
		Vector<Boolean> visited = new Vector<Boolean>();
		visited.setSize(nodes.size());
		for(int i =0; i<nodes.size(); i++){
			visited.set(i, false);
...
//		System.out.println(betweenEdges.size());
	...
	public void printGraph(){
		for(int i = 1; i <= numNodes; i++){
			...
			while (iter.hasNext()) {
				int index = iter.next();
				System.out.println(" Dest: " + edges.get(index).kraj + " W: "
