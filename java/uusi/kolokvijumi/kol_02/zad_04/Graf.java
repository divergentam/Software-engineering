...
public class Graf {
	...
	public void addEdgeInNonOrientedGraph(int start, int end, int weight){
		if(!doesEdgeExist(start,end,weight)){
			numEdges++;
			edges.add(new Edge(start, end, weight));
			nodes.get(start).add(edges.size() - 1);
		}if(!doesEdgeExist(end,start,weight)){
			numEdges++;
	...
	private boolean doesEdgeExist(int start, int end, int weight){
		boolean exist = false;
